 ...
 * @tutorial TODO 文档
 *
 * @property {Number} duration = [250|500] 滑动动画时长，默认为500
 * @property {String} indicatorColor = [#868e96] 指示点颜色，默认为#868e96
 * @property {Boolean} indicatorDots = [true|false] 显示面板指示点，默认为true显示
 * @property {String} indicatorActiveColor = [white] 当前选中的指示点颜色，默认为white白色
 *
 * @property {String} activeClass  swiper-item 可见时的 class，仅支持支付宝小程序
 * @property {String} changingClass  acceleration 设置为 {{true}} 时且处于滑动过程中，中间若干屏处于可见时的class，仅支持支付宝小程序
 *
 * @property {Number} current = [0] 当前所在滑块的 index，默认为0
 ...
 *
 *
 * @property {Boolean} circular = [true|false] 是否采用衔接滑动，即播放到末尾后重新回到开头，默认为true，衔接滑动
 *
 ...
 *
 *
 * @property {Boolean} acceleration = [true|false] 当开启时，会根据滑动速度，连续滑动多屏，默认为false，仅支持支付宝小程序
 *
 * @property {Boolean} disableProgrammaticAnimation = [true|false] 是否禁用代码变动触发 swiper 切换时使用动画，默认为false，仅支持支付宝小程序
 *
 * @property {Number} displayMultipleItems = [1] 同时显示的滑块数量，默认为1，支付宝小程序不支持
 *
 * @property {Boolean} skipHiddenItemLayout = [true|false] 是否跳过未显示的滑块布局，设为 true 可优化复杂情况下的滑动性能，但会丢失隐藏状态滑块的布局信息，默认为false，仅支持App、微信小程序
 *
 * @property {Boolean} disableTouch = [true|false] 是否禁止用户 touch 操作，默认为false，不禁止
 *
 * @property {Boolean} touchable = [true|false] 是否监听用户的触摸事件，只在初始化时有效，不能动态变更，默认为true，监听用户触摸
 *
 * @property {Boolean} easingFunction = [default|linear|easeInCubic|easeOutCubic|easeInOutCubic] 指定 swiper 切换缓动动画类型，仅支持微信小程序、快手小程序，默认为default
 *
 * @property {Object} img 图片属性，可参考 t-img 组件props
 *
 * @property {Array} list = [] 轮播图列表，元素必须是包含url的对象
 *
